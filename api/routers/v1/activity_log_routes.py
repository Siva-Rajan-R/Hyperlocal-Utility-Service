...
@router.get('/{shop_id}')
async def get_activity_logs(shop_id: str, limit: int = 50, offset: int = 0, q: Optional[str] = Query(None), from_date: Optional[str] = Query(None), to_date: Optional[str] = Query(None), exclude_system: bool = Query(False)):
    logs = await ActivityLogReadDbRepo.get_logs(shop_id=shop_id, limit=limit, offset=offset, query=q, from_date=from_date, to_date=to_date, exclude_system=exclude_system)
    return {
        "detail": {
            "status_code": 200,
            "success": True,
